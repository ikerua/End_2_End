"""
train_base_fase2.py - Training Script for Whisper Base (Basque)
ENCODER UNFROZEN (FASE 2) + RESUME FROM CHECKPOINT
"""
    ...
    def __call__(self, features: List[Dict[str, Union[List[int], torch.Tensor]]]) -> Dict[str, torch.Tensor]:
        ...
        for idx, feature in enumerate(features):
            ...
            try:
                ...
                print(f" [DataCollator] ERROR en audio {idx}: {e}. Sustituyendo por dummy Mel.")
                num_vacios += 1
                num_canales = self.processor.feature_extractor.feature_size
                extracted = np.zeros((num_canales, 3000), dtype=np.float32)
...
class WhisperLightning(pl.LightningModule):
    def __init__(self, model_path, learning_rate, weight_decay, warmup_steps, max_steps):
        ...
        # FASE 2: ENCODER DESCONGELADO Y PROTECCION DE MEMORIA
    ...
    def training_step(self, batch, batch_idx):
        ...
        if is_master and (batch_idx < 5 or batch_idx % 50 == 0):
            ...
            print(f" [GPU 0] Errores DataCollator: {errores_collator}")
            print(f" [GPU 0] Features: {input_features.shape}, Labels: {labels.shape}")
            print(f" [GPU 0] Etiquetas vacias: {vacias}/{total_etiquetas}")
            canales_esperados = self.model.config.num_mel_bins
            if input_features.shape[1] != canales_esperados:
                print(f" [FATAL GPU 0] ERROR DE CANALES. Esperados {canales_esperados}, hay {input_features.shape[1]}")

        porcentaje_basura = vacias / total_etiquetas
        
        if porcentaje_basura > 0.75:
            if is_master: 
                print(f" [ALERTA] Batch {batch_idx} toxico ({porcentaje_basura*100:.1f}%). Saltando.")
        ...
        if is_master and (batch_idx < 10 or batch_idx % 10 == 0):
             print(f" [Step {batch_idx}] Loss: {loss.item():.5f}")

        if torch.isnan(loss) or torch.isinf(loss):
            if is_master: print(f" [ERROR] Loss NaN en step {batch_idx}. Aplicando Dummy.")
            dummy_loss = sum([p.sum() for p in self.parameters() if p.requires_grad]) * 0.0 + 1.5
            return dummy_loss
...
def main():
    ...
    parser.add_argument("--model_path", type=str, default="/gpfs/projects/ehpc485/tesi681824/transcriptor/modelo_whisper_base")
    ...
    parser.add_argument("--output_dir", type=str, default="/gpfs/scratch/ehpc485/tesi681824/whisper_eu_out_base/checkpoints")
    parser.add_argument("--log_dir", type=str, default="/gpfs/scratch/ehpc485/tesi681824/whisper_eu_out_base/logs")
    ...
    checkpoint_best = ModelCheckpoint(
        ...
        filename="whisper-base-epoch={epoch:02d}-wer={val_wer:.4f}",
        monitor="val_wer",
        mode="min",
        save_top_k=3,
        save_last=True,
    ...
    print("================================================================")
    print(" REANUDANDO ENTRENAMIENTO DESDE CHECKPOINT (FASE 2: ENCODER ACTIVO)")
    print("================================================================")

    ruta_checkpoint = "/gpfs/scratch/ehpc485/tesi681824/whisper_eu_out_base/checkpoints/last.ckpt"
    ...
    final_path = os.path.join(args.output_dir, "whisper-base-euskera-final")
