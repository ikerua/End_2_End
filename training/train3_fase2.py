...
def main():
    ...
    swa = StochasticWeightAveraging(swa_lrs=5e-7)
    ...
    trainer = pl.Trainer(
        ...
        callbacks=[checkpoint_best, early_stopping, lr_monitor, swa], logger=csv_logger,
    ...
    ruta_checkpoint = "/gpfs/scratch/ehpc485/tesi681824/whisper_eu_out/checkpoints/whisper-best-epoch=epoch=00-wer=val_wer=0.0802.ckpt"

    model = WhisperLightning.load_from_checkpoint(
        ruta_checkpoint,
        model_path=args.model_path, 
        learning_rate=5e-7, # MICRO LEARNING RATE
        weight_decay=args.weight_decay,
        warmup_steps=500,
        max_steps=args.max_steps
    )
    
    # IMPORTANTE: En Fase 2 NO le pasamos el ckpt_path al trainer.fit(), 
    # porque queremos que el Optimizador empiece fresco y de cero, 
    # solo heredamos la "inteligencia" (los pesos) del modelo arriba.
