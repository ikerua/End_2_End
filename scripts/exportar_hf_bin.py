...
def main():
    ...
    try:
        ...
        # Ajustar el tamaño del vocabulario si el checkpoint tiene tokens extra (e.g. 51866 vs 51865)
        if "state_dict" in ckpt and "model.proj_out.weight" in ckpt["state_dict"]:
            ckpt_vocab_size = ckpt["state_dict"]["model.proj_out.weight"].shape[0]
            if ckpt_vocab_size != model.config.vocab_size:
                print(f"Ajustando tamaño del vocabulario de {model.config.vocab_size} a {ckpt_vocab_size}...")
        ...
        # Intentar cargar con strict=False por precaución
        model.load_state_dict(state_dict_hf, strict=False)
    ...
    # Guardar modelo y procesador (Aseguramos que guarde el .bin antiguo para mayor compatibilidad)
    model_lightning.model.save_pretrained(args.output_dir, safe_serialization=False)
